...
const Modal = ({children, type, title, show, buttons, className }:ModalProps) => {

    return (
        <div 
            className={`${show? 'block':'hidden'} absolute z-10`}
            aria-labelledby="modal-title" 
            role="dialog" 
            aria-modal="true"
        >
            <div className={`bg-gray-800/40 fixed inset-0 bgtexto-label/75 transition-opacity`} aria-hidden="true"></div>

            <div className={` fixed inset-0 z-10 w-screen overflow-y-auto`}>
                <div className={` flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0`}>
                    <div className={`${className} bg-white  absolute top-1/4 transform overflow-hidden rounded-lg text-left shadow-md transition-all sm:my-8 min-w-sm md:min-w-lg border border-borda`}>
                        <div className="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div>
                                ...
                                <div className="mt-3 text-center sm:mt-0 sm:text-left">
                                    ...
                                    <div className="mt-2">
                                        <div className="text-texto-label">
                                            {children}
                                        </div>
                                    </div>
                                </div>
