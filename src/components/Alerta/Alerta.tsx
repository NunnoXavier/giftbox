...
const Alerta = ({ children,titulo, visivel, indice }:{ children?:ReactNode,titulo?:string, visivel?:boolean, indice:number  }) => {
    ...
    return (
        <div className={`${open? 'top-10 opacity-100': '-top-50 opacity-0'}  
            absolute bg-white/90 shadow-md left-1/2 md:left-10/12 -translate-x-1/2  
            w-md h-30 border border-texto-alerta/50 rounded-lg transition-all 
